...
启发式扫描（空词与解释词、机制词、绝对化的空或黑、非特写镜头里的尺度名词、同一镜里的远处与贴镜、关键动作紧挨整幅遮挡、发力过程写法、静止、景别、焦点落点、弱运镜措辞、动作密度、素材重复绑定、跨段重复长句、风格段里的时序与运镜、主体段里的动作与调度、场景段与风格段里的角色活动、总括保证句）只给警告。动作密度：节拍数用时序词粗估（“先”“同时”不另算一拍），平均不到 0.5 秒一拍（1 秒里超过 2 拍）才提醒，正好 0.5 秒不报；
...
v18 的五类提醒（词表都在脚本头部常量里，旁注「可调」）：
  ...
  尺度名词（织纹、纤维、毛孔、抽丝……）出现在**没有**特写 / 大特写 / 微距字样的镜头正文里时提醒，等于要求模型换景别去拍
...
v21 两类提醒：关键动作（松手、脱手……）和整幅遮挡（整幅被、糊住、全黑……）写在同一镜的同一句或相邻句（两句各带第N秒
  且相差 ≥1 秒视为已错开，遮挡词前两个字里有否定字“不 / 没 / 未 / 无”就不算；运镜不在这条里扫，呈现关键事件的运镜可以同一瞬）；发力过程写法（先转肩、再转胯、腰先转、肩膀慢半拍、引到最后；
...
v23 三类提醒（只提醒不拦截，词表 SUBJECT_*_RE / BG_ACT / SCENE_*_RE / GUARANTEE_*_RE / FRAME_*_RE 在头部常量里、可调）：
  ...
  衣物、头发、持物的可见结果写进镜内那一拍，每拍一处（成文主规则第 3 条；镜头正文里的这类句子不扫）；
  ...
  全片不换手的道具归属（全程用右手握棍）、镜头性格、伤势与外形状态（湿透）不算。改稿时父稿原句照报并标“父稿原有”；--partial 不报父稿原句；
  锁定文字与 --asks 关键词不报。
否定句：四段稿默认预算 0 条自写否定（固定句不计）。全文（固定句与引号内台词除外）里句首是
  `不出现|不添加|不得|不要|不能|不许|不可|禁止|避免` 的句子逐句给**提醒**（不是错误）；用 --negative-exception 逐句点名的不再提醒。
  writing-rules 第 62 条列的已试例外多是句中否定（开始的一秒画面是纯黑，看不到任何轮廓、光点或亮边），脚本不逐句提醒；
  点名时稿里有这一整句（单独成句）就不报错。
...
# 出现在别的景别里等于要求模型换景别去拍。只扫镜头正文；主体、场景、风格段描述表面属性的材质词不受第 5 条约束，不扫。两张词表可调。
...
GUARANTEE_EXEMPT_RE = re.compile(
    r"只有|唯一|仅有|不换手|不离手|不离身|[左右双]手(?:握|拿|持|提|攥|抓|拎|捧|执)|(?:是|为)同一|同一(?:主体|身份)"
...
def main():
    ...
    ap.add_argument("--negative-exception", default="", help="要逐句点名的必要否定句本身，多句用“；”分开。四段稿：点名的否定句（镜内，或用户要求全片静音时写在主体段的那一句）不再提醒（其余每句给一条提醒），writing-rules 第 62 条的已试例外（句中否定）稿里有这一整句就不报错；五段 / 六段 / 继承旧稿：每句必须与结尾段里一条独立否定条款整句一致，数量要覆盖超出 4 条预算的部分。最终放行仍须审查")
    a = ap.parse_args()
    ...
    for k, (h, body_lines, _t) in enumerate(blocks):
        ...
        if h[3] is not None and h[4] is not None and h[4] > h[3]:
            dur = h[4] - h[3]
            # 句首已算一拍，“先”不另算；“同时”是并行叠加（writing-rules 第 53 条），不算新的一拍
            beats = 1 + len(re.findall(r"接着|紧接着|随后|然后|最后|再(?=[抬伸迈转抢踏收送压])", body))
            if dur / beats < 0.5 - 1e-9:   # 正好 0.5 秒一拍不报（L064 口径）
                warnings.append(f"{tag} 约 {beats} 个动作节拍挤在 {dur:g} 秒里（平均不到 0.5 秒一拍，即 1 秒里超过 2 拍；按时序词粗估）；"
    ...
    for k, (h, body_lines, _t) in enumerate(blocks):
        ...
        # 尺度名词与距离链（都只扫镜头正文，台词不算；主体、场景、风格段的材质词本来就不在这里）
        ...
        for s in split_sentences(subj):
            ...
            if acts:
                ...
                warnings.append(f"主体段这句写了动作、随动、持物状态或调度：{''.join(f'「{w}」' for w in acts[:5])}"
                                ...
                                f"每拍一处{prop}（seedance-format 第 1 节主体段；writing-rules 成文主规则第 3、7 条）"
                                + (PARENT_NOTE if _parent(s) else ""))
            elif gs:

...
当前为 v24：运镜可以和关键事件同一瞬去呈现它，那一瞬只避开整幅遮挡和抢同一主体或道具、朝镜头扑来的大动作；每拍可写一处衣物、头发或持物的可见结果；改动作设计时整拍重写；有意的纯黑和几条已试的否定写法列为可用例外。建立在 v23（主体段只写是谁、长什么样、有几个；总括保证句不代替镜内描述）、v22 讲戏口吻（术语后紧跟半句看得见的结果、画面坐标、短句按时间往下说、镜内不加标签）与 v21 写法分主次（每拍三样、关键事件在明处、装饰层不逐拍、发力只写结果）之上。入口只保留关键底线、任务路由和交付条件；格式、写法、工艺、改稿、检查各有唯一详细归属。默认轻量路径与严格审触发条件未改变。
...
tests/run_check_tests.py     check_prompt 回归 + 经验库前缀强制 + 案例库体检（396 项）

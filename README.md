...
当前为 v25：全库审查定案——规则本体消掉 20 处互相矛盾、成文主规则第 1/3/7 条改成一读就懂的写法、SKILL.md 只留摘要；检查器对讲戏口吻生效（按句数估拍、只认摄影运动、关键事件词表、站位保证全稿扫），修掉两处错误级误报；工艺卡与词库示范句全部按现行口径重写；钩子代跑一律四段。上一版 v24 的口径全部保留，见 CHANGELOG。

...
当前为 v28：写完稿后固定过一遍「压缩审校」（三问删复述、常识、虚词；切线、坐标、秒数、关键事件、情绪方向句等承重内容不碰；写到关键事件能看清为止，其余交给模型），加「松紧旋钮」（默认 / 更放 / 更控），检查器多三条只提醒不拦截的复读、虚词、密度提示（密度参考线默认 200 字/秒，待 A/B 实测）。v27 词库 11 张 988 条与拔高机制、v26 去处表、v25 全库审查定案全部保留，见 CHANGELOG。
